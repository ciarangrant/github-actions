...
then
  echo "$RELEASE_NOTES doesn't contain unreleased changes"
  exit 1
else
  echo "$RELEASE_NOTES contains unreleased changes: $UNRELEASED_CHANGES"
fi
...
then
  ...
  echo "$RELEASE_NOTES contains update type ${UPDATE_TYPE}"
else
